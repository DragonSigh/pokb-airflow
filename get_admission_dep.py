...
def start_analyze():
    ...
    df_perv = df \
        ...
        .agg({'ФИО': 'count', 'po_done': 'sum', 'napr_done': 'sum', 'correct_time': 'sum'}) \
        .assign(Заполнен_осмотр         = lambda x: round(100*x['po_done'] / x['ФИО'],0).astype(int),
                Создано_направление     = lambda x: round(100*x['napr_done'] / x['ФИО'],0).astype(int),
                Время_осмотра_корректно = lambda x: round(100*x['correct_time'] / x['ФИО'],0).astype(int)) \
        [['ФИО', 'Заполнен_осмотр', 'Создано_направление', 'Время_осмотра_корректно']] \
        .reset_index() \
        .rename(columns = {'ФИО': 'Пациентов', 
        ...
        [['ФИО Врача', 'Приемное_отделение', 'Пациентов', 'Заполнен осмотр, %', 
        'Создано направление, %', 'Время осмотра корректно, %']]
     
    utils.save_to_excel(df_perv,
        os.path.join(EXPORT_PATH, 'Заполнение первичного осмотра в приемном по врачам.xlsx'),
    )
    ...
    utils.save_to_excel(df_perv_osp,
        os.path.join(EXPORT_PATH, 'ОСП Заполнение первичного осмотра в приемном по врачам.xlsx'),
    )
    ...
    df_stat = df \
        ...
        .agg({'Время пребывания в приемном, мин, от времени поступления до перевода в профильное отделение': 'mean'}) \
        .apply(round) \
        .rename(columns={'Время пребывания в приемном, мин, от времени поступления до перевода в профильное отделение': 
                        'Среднее время пребывания в приемном'}) \
        .reset_index()

    utils.save_to_excel(df_stat,
    ...
    utils.save_to_excel(df_stat_osp,
